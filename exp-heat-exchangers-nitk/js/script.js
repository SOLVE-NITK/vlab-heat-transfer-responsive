...
function drawGradient(){
  ...
  ctx.fillRect(158, 78, 305, 28);
   ...
   if(currentVoltage == "1"){
    ...
    //outer tube gradient
    document.querySelector("#info").innerHTML = "Counter Flow";
    var x = 150*time1;
    var y = 500-x;
    // Create gradient
...
// initial model
function drawModel() {
  ctx.clearRect(0, 0, 490, 400); //clears the complete canvas#simscreen everytime
...
function validateAnswer3() {
  ...
  if (!answer) {
    correctAnswer.classList.remove("hide");
    unit.innerHTML += " <span class='wrong'>&#x2717;</span>";
    correctAnswer.innerHTML = `<span class='correct'>Correct Answer </span>= ${u} `;
  } else if (answer) {
    correctAnswer.classList.add("hide");
    unit.innerHTML += " <span class='correct'>&#x2713;</span>";
  }
